...
function WorkerChild (eventDest, filename) {
  ...
  server.addListener("listening", function () {
    ...
    self.child = child_process.spawn("node", [self.filename].concat(WORKER_PARAS).concat(["-socket", socketFilename]));
...
WorkerChild.prototype = {
  ...
  terminate: function () {
    ...
    this.socket.end();
    this.child.kill();
    debug("Termindated "+terminateCount++)
  }
};
